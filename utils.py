...
def per_node_delayedTX_plot(data: np.ndarray, xlabel: str, ylabel: str, title: str, dirstr: str, avg_window: int = 1000, legend_loc: str = 'upper right', modes = None, step=STEP, figtxt = ''):
    fig, ax = plt.subplots(figsize=(8,4))
    ax.grid(linestyle='--')
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
    ...
    for NodeID in range(NUM_NODES):
        if MODE[NodeID] in modes and np.any(data[:, NodeID]):
            ax.plot(np.arange(avg_window*STEP, SIM_TIME, STEP), data[avg_window:,NodeID], color=colors[MODE[NodeID]])
