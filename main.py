...
from utils import all_node_plot, per_node_barplot, per_node_plot, per_node_plotly_plot, plot_cdf, per_node_rate_plot, scaled_rate_plot, per_node_delayedTX_plot
        ...
        for i in range(TimeSteps):
            ...
            for NodeID, Node in enumerate(Net.Nodes):
                ...
                if len(Node.Inbox.DelayedMessTime)!=0:
                    ...
                    per_node_results['Maxtime of delayed message'][mc][i,NodeID] = 0   

                if len(Node.Inbox.Malicioustxtime)!=0:
                    per_node_results['Maxtime of malicious delayed message'][mc][i,NodeID] = max(Node.Inbox.Malicioustxtime)     
                else:
...
def plot_results(dirstr, per_node_result_keys):
 ...
 # all_node_plot(per_node_results['Number of confirmed Messages'].sum(axis=1), 'Time (sec)', 'Number of confirmed Messages', '', dirstr+'/plots/Allconfirmed.png')
   ...
   # per_node_delayedTX_plot(per_node_results['Maxtime delayed tx NodeID'], 'Time (sec)', 'DelayedTXNodeID', '', dirstr)
    per_node_delayedTX_plot(per_node_results['Maxtime of malicious delayed message'], 'Time (sec)', 'MaliciousDelayedTXTime', '', dirstr)
